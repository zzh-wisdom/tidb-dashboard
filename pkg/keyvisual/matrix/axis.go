...
func (axis *Axis) Reduce(strategy Strategy, newKeys []string) Axis {
	keys := axis.Keys
...
func (axis *Axis) GetFocusRows(strategy Strategy, threshold uint64) (count int) {
	start := 0
...
func (axis *Axis) Divide(strategy Strategy, target int) Axis {
	...
	for lowerThreshold < upperThreshold {
		...
		if axis.GetFocusRows(strategy, mid) > targetFocusRows {
	...
	focusRows := axis.GetFocusRows(strategy, threshold)
